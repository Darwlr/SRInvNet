...
from codes.models.modules.loss import ReconstructionLoss, Gradient_Loss, SSIM_Loss

import numpy as np
import matplotlib
matplotlib.use('TkAgg')
import matplotlib.pyplot as plt
from dataset.Bicubic import *
import cv2
...
class InvDN_Model(BaseModel):
    ...
    def feed_data(self, data):
        self.ref_L = data[1].to(self.device)  # LQ  [2, 1, 128, 128]
        self.real_H = data[3].to(self.device)  # GT                             [2, 1, 256, 256]
        self.noisy_H = data[2].to(self.device)  # GT  [2, 1, 256, 256]
    ...
    def optimize_parameters(self, step):
        ...
        l_forw_ce = 0
        l_forw_fit = self.loss_forward(self.output[:, :1, :, :], LR_ref)


        # backward
        gaussian_scale = self.train_opt['gaussian_scale'] if self.train_opt['gaussian_scale'] != None else 1     # 1
        y_ = torch.cat((self.output[:, :1, :, :], gaussian_scale * self.gaussian_batch(self.output[:, 1:, :, :].shape)), dim=1)      # [8, 12, 72, 72]
    ...
    def test(self, self_ensemble=False):
        ...
        with torch.no_grad():
            if self_ensemble:
                ...
                y_forw = torch.cat(
                    (output[:, :1, :, :], gaussian_scale * self.gaussian_batch(output[:, 1:, :, :].shape)), dim=1)
        ...
        with torch.no_grad():
            if self_ensemble:
                ...
                for i in range(sample_num):
                    y_forw = torch.cat(
                        (output[:, :1, :, :], gaussian_scale * self.gaussian_batch(output[:, 1:, :, :].shape)), dim=1)
                    fake_Hs.append(self.netG(x=y_forw, rev=True)[:, :1, :, :])
    ...
    def forward_x8(self, x, forward_function, gaussian_scale):
        ...
        for data in lr_list:
            ...
            y_forw = torch.cat((data[:, :1, :, :], gaussian_scale * self.gaussian_batch(data[:, 1:, :, :].shape)), dim=1)
